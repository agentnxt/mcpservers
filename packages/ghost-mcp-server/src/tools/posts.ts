...
export function registerPostTools(server: McpServer, client: GhostClient) {
  server.tool(
    "ghost_list_posts",
    ...
    async ({ limit, page, filter, fields, formats, include, order, status }) => {
      ...
      const result = await client.get("/ghost/api/admin/posts/", params);
  ...
  server.tool(
    ...
    async ({ id, formats, include }) => {
      ...
      if (formats) params.formats = formats;
      if (include) params.include = include;
      const result = await client.get(`/ghost/api/admin/posts/${id}/`, params);
      return jsonResult(result);
    }
  );
  ...
  server.tool(
    ...
    async ({ title, html, mobiledoc, status, tags, featured, excerpt, published_at }) => {
      ...
      const result = await client.post("/ghost/api/admin/posts/", { posts: [post] });
      return jsonResult(result);
    }
  );
  ...
  server.tool(
    ...
    async ({ id, updated_at, title, html, status, tags, featured, excerpt }) => {
      ...
      if (featured !== undefined) post.featured = featured;
      if (excerpt) post.excerpt = excerpt;
      const result = await client.put(`/ghost/api/admin/posts/${id}/`, { posts: [post] });
      return jsonResult(result);
    }
  );
  ...
  server.tool(
    ...
    async ({ limit, include }) => {
      ...
      if (limit !== undefined) params.limit = String(limit);
      if (include) params.include = include;
      const result = await client.get("/ghost/api/admin/tags/", params);
      return jsonResult(result);
    }
  );
}
